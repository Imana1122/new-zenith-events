...
const EventDetails = () => {
  ...
  return (
    <DefaultLayoutComponent>
      ...
      <div className="bg-white rounded md:px-2 md:m-5 sm:m-0 px-4">
        {/* Event image */}
        <div className="flex flex-col md:flex-row md:justify-center md:items-center mb-4 overflow-hidden md:space-x-10">
          <div className="w-full h-full md:full sm:w-1/2">
          ...
          {/* Event information */}
          <div className="md:w-[900px] flex flex-col items-start space-y-3 md:space-y-5 md:m-auto text-2xl flex-1">
            <h2 className="text-2xl sm:text-lg md:text-4xl text-left text-purple-900">{selectedEvent?.title}</h2>
            <p className="text-left text-xl md:text-2xl ">{selectedEvent?.workshop}</p>
            ...
            <div className="text-left text-sm md:text-xl w-full space-x-3 items-center flex"><CiLocationOn/><p>{selectedEvent?.address}</p> </div>
        ...
        <Typography
            ...
            sx={{ color: 'purple', textAlign: { md: 'left', sm: 'left',xs:'center' }, mb: 2 }}
        >
            Training Details
        </Typography>
